...
static __device__ __forceinline__ void flash_attn_ext_load_tile(
        ...
        const int stride_KV,
        const int i_sup,
        const int dim2_offset) {
    if constexpr (type_KV == GGML_TYPE_F16) {
        flash_attn_ext_f16_load_tile<stride_tile, nwarps, nbatch_fa, false, oob_check>(
            reinterpret_cast<const half2 *>(KV) + dim2_offset, tile_KV, D2, stride_KV / sizeof(half2), i_sup);
    } else if constexpr (type_KV == GGML_TYPE_Q8_0) {
        constexpr int nthreads = nwarps * 32;
        const int tid = threadIdx.y * 32 + threadIdx.x;
        const int elements = nbatch_fa * D2;
        for (int index = tid; index < elements; index += nthreads) {
            const int row = index / D2;
            const int dim2 = index % D2 + dim2_offset;
            half2 value = make_half2(0.0f, 0.0f);
            if (!oob_check || row < i_sup) {
                const int dim = 2 * dim2;
                const auto * blocks = reinterpret_cast<const block_q8_0 *>(KV + (size_t) row * stride_KV);
                const block_q8_0 & block = blocks[dim / QK8_0];
                const int q = dim % QK8_0;
                const float d = block.d;
                value = make_half2(d * block.qs[q], d * block.qs[q + 1]);
            }
            tile_KV[(index / D2) * stride_tile + index % D2] = value;
        }
    } else {
        static_assert(type_KV == GGML_TYPE_F16 || type_KV == GGML_TYPE_Q8_0, "unsupported FlashAttention KV type");
    }
}
...
static __device__ __forceinline__ void flash_attn_ext_f16_iter(
    ...
    constexpr int  nstages         = type_K == GGML_TYPE_F16 && type_V == GGML_TYPE_F16 ? ggml_cuda_fattn_mma_get_nstages(DKQ, DV, ncols1, ncols2) : 0;
    ...
    if constexpr (nstages > 1) {
        ...
        flash_attn_ext_f16_load_tile<stride_tile_V, nwarps, nbatch_fa, use_cp_async, oob_check>
            (reinterpret_cast<const half2 *>(V_data + int64_t(k_VKQ_0)*stride_V), tile_V, nbatch_V2, stride_V / sizeof(half2), k_VKQ_sup);
    } else {
        ...
        if constexpr (nstages <= 1) {
            ...
            if constexpr (type_K == GGML_TYPE_F16) {
                flash_attn_ext_f16_load_tile<stride_tile_K, nwarps, nbatch_fa, use_cp_async, oob_check>(
                    reinterpret_cast<const half2 *>(K_data + int64_t(k_VKQ_0)*stride_K) + k0_start,
                    tile_K, k0_diff, stride_K / sizeof(half2), k_VKQ_sup);
            } else {
                flash_attn_ext_load_tile<type_K, stride_tile_K, nwarps, nbatch_fa, oob_check>(
...
#pragma unroll
    ...
    if constexpr (nstages > 1) {
        ...
        if (!last_iter) {
            ...
            flash_attn_ext_f16_load_tile<stride_tile_K, nwarps, nbatch_fa, use_cp_async, oob_check>
                (reinterpret_cast<const half2 *>(K_data + int64_t(k_VKQ_0 + nbatch_fa)*stride_K), tile_K, nbatch_K2, stride_K / sizeof(half2), k_VKQ_sup);
        }
    }
        ...
        if constexpr (nstages <= 1) {
            ...
            if (!V_is_K_view || i0_stop > 2*nbatch_K2) {
                ...
                if constexpr (type_V == GGML_TYPE_F16) {
                    flash_attn_ext_f16_load_tile<stride_tile_V, nwarps, nbatch_fa, use_cp_async, oob_check>(
                        reinterpret_cast<const half2 *>(V_data + int64_t(k_VKQ_0)*stride_V) + i0_start/2,
                        tile_V, i0_diff/2, stride_V / sizeof(half2), k_VKQ_sup);
                } else {
                    flash_attn_ext_load_tile<type_V, stride_tile_V, nwarps, nbatch_fa, oob_check>(
...
#pragma unroll
    ...
    if constexpr (nstages > 1) {
        ...
        flash_attn_ext_f16_load_tile<stride_tile_K, nwarps, nbatch_fa, use_cp_async, oob_check>
            (reinterpret_cast<const half2 *>(K_data + int64_t(kb0)*nbatch_fa*stride_K), tile_K, nbatch_K2, stride_K / sizeof(half2), k_VKQ_sup);
    }
...
static __global__ void flash_attn_ext_f16(
    ...
    const int stride_V = V_is_K_view ? stride_K : nb21;
