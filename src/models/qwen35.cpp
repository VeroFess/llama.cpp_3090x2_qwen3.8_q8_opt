...
static void detect_recurrent_layers_qwen35(llama_hparams & hparams, llama_model_loader & ml, llm_arch arch) {
...
void llama_model_qwen35::load_arch_hparams(llama_model_loader & ml) {
    ...
    // MTP layers are dense attention-only and must not use recurrent memory.
    for (uint32_t i = hparams.n_layer_kv_from_start; i < hparams.n_layer; ++i) {
    ...
    switch (hparams.n_layer - hparams.nextn_predict_layers) {
        ...
        case 66: type = LLM_TYPE_27B; break;
...
ggml_tensor * llama_model_qwen35::graph::build_layer_attn_linear(
    ...
    if (!keep_intermediates) {
        // Update convolution state cache. In tree mode with linear parents, the last
        // d_conv - 1 columns are correct; tree_rollback overwrites branched cases.
    ...
    if (!keep_intermediates) {
        ...
        // Update the recurrent states. Tree rollback overwrites branched cases.
